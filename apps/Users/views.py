...
class CustomBackend(ModelBackend):
    def authenticate(self, username=None, password=None, **kwargs):
        try:
            user = UsersProfile.objects.get(Q(username=username) | Q(email=username))
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if login_form.is_valid():  # is_valid()判断是否通过表单验证
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if UsersProfile.objects.filter(email=user_name):
    ...
    def get(self, request, active_code):
        ...
        if all_records:
            for recode in all_records:
                email = recode.email
                user = UsersProfile.objects.get(email=email)
                user.is_active = True
                user.save()
        else:
